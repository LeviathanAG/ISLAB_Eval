...
def measured(call,repeats):
    """Run call repeatedly and return its result plus median milliseconds."""
...
def save(rows,path):
    """Save measurement dictionaries as CSV; MODIFY path to choose output."""
...
def plot(rows,path,backend):
    """Plot encryption times; lib uses matplotlib, pure writes simple SVG."""
    totals={}
    for row in rows:
...
def symmetric_benchmark(backend='lib',repeats=20,output='results/symmetric.csv'):
    """Compare DES and AES-128/192/256 over five modes and messages.

    MODIFY messages, algorithm/size pairs, modes, repeats, or output filename.
    Keep the round-trip assertion so a timing result is accepted only if
...
def file_benchmark(backend='lib',sizes=(1024,10240,1048576,10485760),repeats=3,algorithms=('rsa','ecc','ec-elgamal'),output='results/files.csv'):
    """Compare hybrid file encryption.

    MODIFY sizes in bytes, algorithm names, repeats, backend, or output file.
    Key generation, encryption, decryption, and envelope overhead are separate.
    """
    rows=[]
...
def main():
    """Read command-line choices; run with --help for modifiable arguments."""
